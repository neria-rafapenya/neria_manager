...
const getEnvChatbotOpenedFlag = (): boolean => {
  const runtime = getRuntimeConfig()?.chatbotOpened;
...
const getEnvCaptchaFlag = (): boolean => {
  const runtime = getRuntimeConfig()?.captchaEnabled;
  if (typeof runtime === "boolean") {
    return runtime;
  }
...
const getEnvChatbotRestrictedFlag = (): boolean => {
  const runtime = getRuntimeConfig()?.chatbotRestricted;
  if (typeof runtime === "boolean") {
    return runtime;
  }
...
const ChatbotLayout = ({
  ...
  useEffect(() => {
    ...
    const siteKey = (getRuntimeConfig()?.recaptchaSiteKey || import.meta.env.VITE_RECAPTCHA_SITE_KEY) as
      | string
      | undefined;
